...
const AuthCallback: React.FC = () => {
  ...
  useEffect(() => {
    const run = async () => {
      try {
        // If Supabase sent an OAuth PKCE code, exchange it for a session
        ...
        // Check for active session (works for magic link/hash tokens too)
        const { data } = await supabase!.auth.getSession()
        if (data.session) {
          setStatus('signed-in')
          // Clean URL (drop query/fragment)
          window.history.replaceState({}, document.title, window.location.pathname)
          navigate(from, { replace: true })
          return
        }

        // No active session: likely just email confirmed. Prompt to sign in.
        setStatus('confirmed')
